      ...
      if (findDecoderByName(impl, codec)) {
        if (codec.hwAccel && hwCapabilityCheck) {
          ...
          if (deviceHasNoHwDecoder(codec.implementationName, nvcodecContext._device_name)) {
            WARN_LOG(
                "Do not list HW decoder " + codec.implementationName + " due to detected device " +
                nvcodecContext._device_name + " does not support this HW decoder");
            continue;
          }
#endif
        }
      ...
      if (findDecoderByName(impl, codec)) {
        if (codec.format == standard) {
          if (codec.hwAccel && hwCapabilityCheck) {
            ...
            if (deviceHasNoHwDecoder(codec.implementationName, nvcodecContext._device_name)) {
              WARN_LOG(
                  "Do not list HW decoder " + codec.implementationName +
                  " due to detected device " + nvcodecContext._device_name +
                  " does not support this HW decoder");
              continue;
            }
#endif
          }

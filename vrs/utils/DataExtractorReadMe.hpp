...
The second line hold’s the record’s payload, as json as well, of course. It contains a single value named `content`, which is an array of json message, each describing a content block. 

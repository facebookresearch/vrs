...
namespace {
void convertPixelGroup(const std::uint8_t* RESTRICT src, std::uint16_t* RESTRICT dst) {
...
void convertVectorized(
    std::uint16_t* RESTRICT dst,
    const std::uint8_t* RESTRICT src,
    const std::size_t widthInPixels,
    const std::size_t heightInPixels,
  ...
  if (contiguous) {
    ...
    for (int row = 0; row < heightInPixels; ++row) {
      const std::uint8_t* RESTRICT srcPtr = src;
...
bool convertRaw10ToGrey10(
  ...
  const std::uint8_t* RESTRICT src = static_cast<const std::uint8_t*>(srcBuffer);
  std::uint16_t* RESTRICT dst = reinterpret_cast<std::uint16_t*>(dstBuffer);

  const std::size_t minStride = (widthInPixels * 10) / 8;
  if (!XR_VERIFY(
          (widthInPixels % 4) == 0,

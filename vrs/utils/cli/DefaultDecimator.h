...
constexpr double kDefaultBucketMaxTimestampDeltaSec = 1.0 / 30.0;

struct DecimationParams {
  DecimationParams() = default;
  DecimationParams(double minIntervalSec, double bucketMaxTimestampDeltaSec)
  ...
  double bucketMaxTimestampDelta = kDefaultBucketMaxTimestampDeltaSec;
};

/// Default decimator handling stream interval & bucket decimation
class DefaultDecimator : public Decimator {
 public:
  DefaultDecimator(FilteredFileReader& filteredReader, const DecimationParams& params);
  DefaultDecimator(
      FilteredFileReader& filteredReader,
      double minIntervalSec,
      double bucketMaxTimestampDeltaSec = kDefaultBucketMaxTimestampDeltaSec)
      : DefaultDecimator(
            filteredReader,
            DecimationParams{minIntervalSec, bucketMaxTimestampDeltaSec}) {}

  static void decimate(FilteredFileReader& filteredReader, const DecimationParams& params);
  static void decimate(
      FilteredFileReader& filteredReader,
      double minIntervalSec,
      double bucketMaxTimestampDeltaSec = kDefaultBucketMaxTimestampDeltaSec) {
    decimate(filteredReader, DecimationParams{minIntervalSec, bucketMaxTimestampDeltaSec});
  }

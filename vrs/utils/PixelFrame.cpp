...
inline uint8_t clip_uint8(int value) {
  ...
  if (srcFormat == PixelFormat::BGR8) {
    ...
    for (uint32_t h = 0; h < getHeight(); h++, srcPtr += srcStride, outPtr += outStride) {
      ...
      for (uint32_t w = 0; w < width / 2; w++, lineSrcPtr += 4, lineOutPtr += 6) {
        ...
        int d = u0 - 128;
        int e = v0 - 128;
        lineOutPtr[2] = clip_uint8((298 * c + 516 * d + 128) >> 8); // blue
        lineOutPtr[1] = clip_uint8((298 * c - 100 * d - 208 * e + 128) >> 8); // green
        lineOutPtr[0] = clip_uint8((298 * c + 409 * e + 128) >> 8); // red
        c = y1 - 16;
        lineOutPtr[5] = clip_uint8((298 * c + 516 * d + 128) >> 8); // blue
        lineOutPtr[4] = clip_uint8((298 * c - 100 * d - 208 * e + 128) >> 8); // green
        lineOutPtr[3] = clip_uint8((298 * c + 409 * e + 128) >> 8); // red
      }
    }

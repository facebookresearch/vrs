...
PixelFormat PixelFrame::getNormalizedPixelFormat(
  ...
  if ((options.semantic == ImageSemantic::ObjectClassSegmentation ||
    ...
    format = PixelFormat::RGB8;
      ...
      format == PixelFormat::BAYER8_GBRG || format == PixelFormat::BAYER10_GBRG) {
    format = PixelFormat::RGB8;
#endif
  } else {
...
bool PixelFrame::inplaceRgbaToRgb() {
  ...
  pixel_conversions::convertRgbaToRgb(
      rdata(), getStride(), wdata(), rgbSpec.getStride(), width, height);
  imageSpec_ = rgbSpec;
...
bool PixelFrame::normalizeFrame(
  ...
  if (srcFormat == PixelFormat::BGR8) {
    ...
    pixel_conversions::upscalePixels16(
        data<uint16_t>(),
        outNormalizedFrame.data<uint16_t>(),
        getWidth() * getHeight() * componentCount,
        bitsToShift);

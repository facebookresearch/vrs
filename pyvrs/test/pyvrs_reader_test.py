...
#     def setUp(self):
#         self.test_file_path = f"test_{__name__}_{os.getpid()}.vrs"
#         if os.path.isfile(self.test_file_path):
#             os.remove(self.test_file_path)
#         self.writer = VRSWriter(self.test_file_path)
#         self.writer.resetNewInstanceIds()

#     def tearDown(self):
#         if os.path.isfile(self.test_file_path):
#             os.remove(self.test_file_path)

#     def fill_out_stream(self, stream: VRSStream):
#         timestamp = datetime.now().timestamp()
#         stream.create_config_record(timestamp, stream.get_config_record_metadata())
#         stream.create_state_record(timestamp)
#         stream.create_data_record(timestamp, stream.get_data_record_metadata(0))
#         self.writer.flush_records(timestamp)

#     def validate_find_streams(self, use_multi_path: bool):
#         reader = SyncVRSReader(
#             self.test_file_path,
#             auto_read_configuration_records=True,
#             multi_path=use_multi_path,
#         )
#         self.assertEqual(reader.find_stream(65500, "name", "sample"), "65500-1")
#         self.assertEqual(len(reader.find_streams(65500)), 4)
#         self.assertEqual(reader.find_streams(65500, "flavor_1"), ["65500-2"])
#         self.assertEqual(reader.find_streams(65500, "flavor_2"), ["65500-3", "65500-4"])
#         self.assertEqual(reader.find_streams(0, "flavor_2"), ["65500-3", "65500-4"])
#         self.assertEqual(reader.get_stream_info("65500-2")["flavor"], "flavor_1")

#     def test_find_streams(self):
#         sample_stream = self.writer.create_stream("sample")
#         sample_stream.set_tag("name", "sample")
#         stream_flavor_1 = self.writer.create_stream("sample_with_flavor_1")
#         stream_flavor_2a = self.writer.create_stream("sample_with_flavor_2")
#         stream_flavor_2b = self.writer.create_stream("sample_with_flavor_2")
#         self.fill_out_stream(sample_stream)
#         self.fill_out_stream(stream_flavor_1)
#         self.fill_out_stream(stream_flavor_2a)
#         self.fill_out_stream(stream_flavor_2b)
#         self.writer.close()

#         self.validate_find_streams(False)
#         self.validate_find_streams(True)


# def character_encoding_recording_path() -> Path:
#     """Path to file with exotic character set"""
#     return Path(pkg_resources.resource_filename(__name__, "test_data/badcharset.vrs"))
...
#     ],
# )
# class BadCharSetTest(unittest.TestCase):
#     def test_open_file(self):
#         reader = SyncVRSReader(
#             character_encoding_recording_path(),
#             auto_read_configuration_records=False,
#             encoding="utf-8-safe",
#             multi_path=self.multi_path,
#         )
#         print(reader)
#         reader._reader.enableAllStreams()
#         streams = reader._reader.getStreams()
#         print(streams)
#         for stream in streams:
#             print(reader._reader.getTags(stream))
...
#     ],
# )
# class TestReadVideoCompressedFiles(unittest.TestCase):
#     def test_sequential_read(self):
#         paths = (
#             [
#                 pkg_resources.resource_filename(
#                     __name__, "test_data/ImageFilterTest-H265-1.vrs"
#                 ),
#                 pkg_resources.resource_filename(
#                     __name__, "test_data/ImageFilterTest-H265-2.vrs"
#                 ),
#             ]
#             if self.multi_path
#             else pkg_resources.resource_filename(
#                 __name__, "test_data/ImageFilterTest-H265.vrs"
#             )
#         )
#         reader = SyncVRSReader(
#             paths,
...
#         filtered_reader = reader.filtered_by_fields(record_types="data")
#         for record in filtered_reader:
#             if not record.image_blocks:
#                 continue
#             self.assertEqual(record.image_blocks[0].shape[0], 480)
#             self.assertEqual(record.image_blocks[0].shape[1], 640)

#     def test_jump_read(self):
#         paths = (
#             [
#                 pkg_resources.resource_filename(
#                     __name__, "test_data/ImageFilterTest-H265-1.vrs"
...
#         reader.set_image_conversion(ImageConversion.DECOMPRESS)
#         filtered_reader = reader.filtered_by_fields(record_types="data")
#         for i in range(0, len(filtered_reader), 5):
#             record = filtered_reader[i]
#             if not record.image_blocks:
#                 continue
#             self.assertEqual(record.image_blocks[0].shape[0], 480)
#             self.assertEqual(record.image_blocks[0].shape[1], 640)
